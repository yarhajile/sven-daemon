...
void RCSwitch::selectPulseLivolo(byte inBit) {
      switch (inBit) {
      case 0: 
       for (byte ii=1; ii<3; ii++) {
        if (this->high == true) {   // if current pulse should be high, send High Zero
          this->sendPulseLivolo(2); 
        } else {              // else send Low Zero
                this->sendPulseLivolo(4);
        }
      ...
      case 1:                // if current pulse should be high, send High One
        if (this->high == true) {
          this->sendPulseLivolo(3);
        } else {             // else send Low One
                this->sendPulseLivolo(5);
        }
        this->high=!(this->high); // invert next pulse
        break;        
      }
...
void RCSwitch::sendPulseLivolo(byte txPulse) {
...
void RCSwitch::sendButton(uint16_t remoteID, byte keycode) {
  for (pulse= 0; pulse <= 180; pulse = pulse+1) { // how many times to transmit a command
      this->sendPulseLivolo(1); // Start  
      ...
      for (i = 16; i>0; i--) { // transmit remoteID
            ...
            this->selectPulseLivolo(txPulse);    
      }
...
void RCSwitch::enableReceive() {
  if (this->nReceiverInterrupt != -1) {
    ...
    // Ok we should add ISR here :
    GPIOISR(this->nReceiverInterrupt, 0, handleInterrupt); 
  }
}
...
RCSwitch main_switch;

extern "C"  //Tells the compile to use C-linkage for the next scope.
{
    void initialize_rcswitch() {
        main_switch = RCSwitch();
    }
    
    void init_receiver(int PIN) {
        pinMode(PIN, INPUT);
        main_switch.enableReceive(PIN);
    }
    
    void init_transmitter(int PIN) {
        pinMode(PIN, OUTPUT);
        main_switch.enableTransmit(PIN);
    }
    
    int get_received_value() {
        int value = -1;
        if (main_switch.available()) {
            //printf("available\n");
            value = main_switch.getReceivedValue();
            main_switch.resetAvailable();
        }
        return value;
    }
    
    int transmit_switch(char* systemCode, int unitCode, int command) {
        switch(command) {
            case 1:
                main_switch.switchOn(systemCode, unitCode);
                break;
            case 0:
                main_switch.switchOff(systemCode, unitCode);
                break;
            default:
                return -1;
        }
