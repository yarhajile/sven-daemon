...
void Livolo::sendButton(uint16_t remoteID, byte keycode) {
...
void Livolo::sendPulse(byte txPulse) {
  ...
  switch(txPulse) { // transmit pulse
   ...
   usleep(500-UMINUS); // 550
   digitalWrite(txPin, LOW);
   break;
   case 2: // "High Zero"
   digitalWrite(txPin, LOW);
   usleep(110-UMINUS); // 110
   digitalWrite(txPin, HIGH);
   break;   
   case 3: // "High One"
   digitalWrite(txPin, LOW);
   usleep(300-UMINUS); // 303
   digitalWrite(txPin, HIGH);
   break;      
   case 4: // "Low Zero"
   digitalWrite(txPin, HIGH);
