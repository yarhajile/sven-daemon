...
class RCSwitchDevice(Base):
  # Don't allow the module factory to execute us as we are handled only 
  # through RCSwitch.
    ...
    for value in values:
      try:
        notice("\tDispatching unit code: %s for '%s' in '%s' at '%s'"
               % (value['unitcode'], value['device_name'],
                  value['location_name'], value['location_group_name']))
