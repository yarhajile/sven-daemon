...
from Sven.Module.BeagleboneBlack.RCSwitchDevice import RCSwitchDevice
...
class RCSwitch(Base):
  ...
  def threadTarget(self, thread):
    """
    This method is the thread target called from the parent addThread() method.
    The parent addThread() method is typically called from our static dispatch
    method.
    """
    ...
    while thread.running == True :
      ...
      if value > 0 :
        ...
        # The 65564 code is erronous and not picked up as we don't have a
        # device associated with it, however, in this case, device.callback()
        # could be called more than once in the 3 second static bounce time
        # we specified.  Modifify this logic to account for that bug.
        for device in self.devices :
          if int(device.unitcode) == value :
            value_found = True


            if last_received_value != value \
              or last_execution_time < time.time() - 3:
      ...
      for module_name, module_items in items:
        for device in module_items:
          if device.__class__.__name__ == 'RCSwitch' \
            ...
            device_exists = True
            device.devices.extend(RCSwitchDevice.dispatch(
                module_factory, db, device, kwargs['device_id']))
